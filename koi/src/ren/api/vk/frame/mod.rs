...
impl Frame {
    ...
    pub fn generator(device: &Device, buffering: u32) -> Vec<Frame> {
        (0..buffering).into_iter().map(|_index| Frame::new(&device)).collect()
    }
...
fn create_fence(device_handle: &DeviceHandle, flags: Option<vk::FenceCreateFlags>) -> vk::Fence {
    ...
    unsafe{ device_handle.create_fence(&create_info, None).expect("koi::ren::vk::Frame - failed to reate Fence") }
}
