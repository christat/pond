...
impl<'a> PipelineBuilder<'a> {
    ...
    pub fn shaders(mut self, vertex: vk::ShaderModule, fragment: Option<vk::ShaderModule>) -> Self {
        self.shader_stages.clear();
        self.shader_stages
            .push(vk::PipelineShaderStageCreateInfo::default().module(vertex).stage(vk::ShaderStageFlags::VERTEX).name(c"main_vs"));
    ...
    pub fn input_topology(mut self, topology: vk::PrimitiveTopology) -> Self {
        self.input_assembly_state = self.input_assembly_state.primitive_restart_enable(false).topology(topology);
        self
    }
    ...
    pub fn polygon_mode(mut self, polygon_mode: vk::PolygonMode) -> Self {
        self.rasterization_state = self.rasterization_state.line_width(1.0).polygon_mode(polygon_mode);
        self
    }

    pub fn cull_mode(mut self, cull_mode: vk::CullModeFlags, front_face: vk::FrontFace) -> Self {
        self.rasterization_state = self.rasterization_state.cull_mode(cull_mode).front_face(front_face);
        self
    }
    ...
    pub fn blending_disabled(mut self) -> Self {
        self.color_blend_attachment = self.color_blend_attachment.color_write_mask(vk::ColorComponentFlags::RGBA).blend_enable(false);
        self
    }
    ...
    pub fn depth_stencil_state(mut self, depth_write_enable: bool, depth_compare_op: vk::CompareOp) -> Self {
...
pub fn load_shader_module(device_handle: &DeviceHandle, shader: &[u8], flags: Option<vk::ShaderModuleCreateFlags>) -> vk::ShaderModule {
    ...
    let create_info = vk::ShaderModuleCreateInfo::default().code(code).flags(flags.unwrap_or_default());
...
pub fn create_pipeline_layout(
    ...
    let create_info =
        vk::PipelineLayoutCreateInfo::default().set_layouts(set_layouts).push_constant_ranges(push_constant_ranges.unwrap_or_default());
...
pub fn create_compute_pipeline(device_handle: &DeviceHandle, shader_module: vk::ShaderModule, layout: vk::PipelineLayout) -> vk::Pipeline {
    let stage = vk::PipelineShaderStageCreateInfo::default().stage(vk::ShaderStageFlags::COMPUTE).name(c"main_cs").module(shader_module);

    let create_infos = [vk::ComputePipelineCreateInfo::default().layout(layout).stage(stage)];
...
pub fn get_rendering_info<'a>(
    ...
    let mut info =
        vk::RenderingInfo::default().render_area(vk::Rect2D::default().extent(extent)).layer_count(1).color_attachments(color_attachments);
...
impl ComputePushConstants {
    ...
    pub fn as_buffer(&self) -> [u8; 64] {
        ...
        [data_0_buffer, data_1_buffer, data_2_buffer, data_3_buffer].as_flattened().try_into().unwrap()
    }
}
