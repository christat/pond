...
fn validate_layers<'a>(entry: &Entry, layers: &[&'a CStr]) -> Result<(), InstanceConfigError<'a>> {
    let instance_layer_properties = unsafe{ entry.enumerate_instance_layer_properties().expect("koi::ren::vk::instance::Config - failed to enumerate instance layer properties") };

    fn validate_layer<'b>(layer: &'b CStr, available_layers: &[vk::LayerProperties]) -> Result<(), InstanceConfigError<'b>> {
        match available_layers.iter().any(|layer_property| layer_property.layer_name_as_c_str().unwrap() == layer) {
            true => Ok(()),
...
fn validate_extensions<'a>(entry: &Entry, extensions: &[&'a CStr]) -> Result<(), InstanceConfigError<'a>> {
    let instance_extension_properties = unsafe{ entry.enumerate_instance_extension_properties(None).expect("koi::ren::vk::instance::Config - failed to enumerate instance extensions properties") };

    fn validate_extension<'b>(extension: &'b CStr, available_extensions: &[vk::ExtensionProperties]) -> Result<(), InstanceConfigError<'b>> {
