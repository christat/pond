...
impl<'a> DescriptorSetLayoutBuilder<'a> {
    ...
    pub fn build<T : vk::ExtendsDescriptorSetLayoutCreateInfo + ?Sized>(
        ...
        if next.is_some() { create_info = create_info.push_next(next.unwrap()); }

        unsafe { device_handle.create_descriptor_set_layout(&create_info, None).expect("koi::ren::vk::descriptor - failed to Create Descriptor Set Layout") }
    }
}
...
impl DescriptorSetAllocator {
    pub fn new(device_handle: &DeviceHandle, max_sets: u32, pool_ratios: &[DescriptorSetPoolSizeRatio]) -> Self {
        let pool_sizes: Vec<_> = pool_ratios.iter().map(|pool_ratio| {
        ...
        let pool = unsafe { device_handle.create_descriptor_pool(&create_info, None).expect("koi::ren::vk::descriptor - failed to Create Descriptor Pool") };
    ...
    pub fn reset_pool(&mut self, device_handle: &DeviceHandle) {
        unsafe { device_handle.reset_descriptor_pool(self.pool, vk::DescriptorPoolResetFlags::empty()).expect("koi::ren::vk::descriptor - failed to Reset Descriptor Pool") };
    }
