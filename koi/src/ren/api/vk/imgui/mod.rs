use crate::{macros::Convert, ren::api::vk::{buffer::Buffer, device::config::QueueFamilyType, image::Image, pipeline}};

use ash::{Device as DeviceHandle, vk::{self, Handle}};
use glam::Vec2;
use gpu_allocator::{vulkan as vka, MemoryLocation};
use imgui::{internal::{RawCast, RawWrapper}, sys::ImDrawIdx, DrawData};
...
pub fn create_render_buffers(device_handle: &DeviceHandle, allocator: &mut vka::Allocator, index: u32) -> RenderBuffers {
    let (index_buffer, index_buffer_allocation) = Buffer::create(device_handle, allocator, IMGUI_DEFAULT_ALLOCATION_SIZE, vk::BufferUsageFlags::INDEX_BUFFER, vk::SharingMode::EXCLUSIVE, &format!("imgui_index_buffer_{}", index), MemoryLocation::CpuToGpu, true);
    let (vertex_buffer, vertex_buffer_allocation) = Buffer::create(device_handle, allocator, IMGUI_DEFAULT_ALLOCATION_SIZE, vk::BufferUsageFlags::VERTEX_BUFFER, vk::SharingMode::EXCLUSIVE, &format!("imgui_vertex_buffer_{}", index), MemoryLocation::CpuToGpu, true);
    (
        index_buffer,
...
impl<'a> ViewportResources {
    pub fn new(device_handle: &DeviceHandle, allocator: &mut vka::Allocator, count: u32) -> Self {
        Self {
            ...
            buffers: (0..count).map(|index| create_render_buffers(device_handle, allocator, index)).collect(),
        }
    }
...
impl Renderer {
    pub fn new(context: &mut imgui::Context, api: &mut super::Renderer, image_count: u32) -> Self {
        ...
        let viewport_resources = ViewportResources::new(&api.device.handle, &mut api.resource_allocator.handle, image_count);
    ...
    pub fn draw(&mut self, context: &mut imgui::Context, api: &mut super::Renderer, command_buffer: vk::CommandBuffer) {
        ...
        if framebuffer_width <= 0.0 || framebuffer_height <= 0.0 { return; }
        ...
        if draw_data.total_vtx_count > 0 {
            ...
            if index_buffer.size < required_index_buffer_size {
                index_buffer_allocation = index_buffer.resize(&api.device.handle, &mut api.resource_allocator.handle, index_buffer_allocation, required_index_buffer_size, &format!("imgui_index_buffer_{}", self.viewport_resources.index));
            }
            ...
            if vertex_bufer.size < required_vertex_buffer_size {
                vertex_buffer_allocation = vertex_bufer.resize(&api.device.handle, &mut api.resource_allocator.handle, vertex_buffer_allocation, required_vertex_buffer_size, &format!("imgui_vertex_buffer_{}", self.viewport_resources.index));
            }
            ...
            index_buffer.upload(indices.as_slice(), &mut index_buffer_allocation, min_alignment);
            vertex_bufer.upload(vertices.as_slice(), &mut vertex_buffer_allocation, min_alignment);
        }

        // setup render state
        setup_render_state(api, draw_data, index_buffer.handle, vertex_bufer.handle, self.pipeline_layout, self.pipeline, command_buffer, framebuffer_width, framebuffer_height);
                    ...
                    match command {
                        imgui::DrawCmd::Elements { count, cmd_params } => {
                            ...
                            unsafe {
                                ...
                                api.device.handle.cmd_bind_descriptor_sets(command_buffer, vk::PipelineBindPoint::GRAPHICS, self.pipeline_layout, 0, &descriptor_sets, &[]);
                                api.device.handle.cmd_draw_indexed(command_buffer, count as u32, 1, (cmd_params.idx_offset + idx_offset) as u32, (cmd_params.vtx_offset + vtx_offset) as i32, 0);
                            };
                        },
    ...
    pub fn drop(&mut self, api: &mut super::Renderer) {
        unsafe {
            api.device.handle.device_wait_idle().expect("koi::ren::vk::imgui - failed to Wait for Device Idle");
            api.device.handle.destroy_image(self.atlas.handle, None);
            let atlas_allocation = self.atlas_allocation.pop_front().unwrap();
            api.resource_allocator.handle.free(atlas_allocation).expect("imgui::ren::vk::imgui - failed to Free ImGui Font Atlas Image");
            api.device.handle.free_descriptor_sets(self.descriptor_pool, &[self.descriptor_set])
                .expect("imgui::ren::vk::imgui - failed to Free ImGui Font Atlas Descriptor Set");
            api.device.handle.destroy_pipeline(self.pipeline, None);
            api.device.handle.destroy_shader_module(self.fragment_shader, None);
            api.device.handle.destroy_shader_module(self.vertex_shader, None);
            api.device.handle.destroy_pipeline_layout(self.pipeline_layout, None);
            api.device.handle.destroy_descriptor_pool(self.descriptor_pool, None);
            api.device.handle.destroy_descriptor_set_layout(self.descriptor_set_layout, None);
            api.device.handle.destroy_sampler(self.texture_sampler, None);
        }
    }
}
...
(
    ...
    let descriptor_set = unsafe { api.device.handle.allocate_descriptor_sets(&allocate_info).expect("koi::ren::vk::imgui - failed to allocate ImGui Descriptor Set")[0] };
    ...
    // Pipeline
    let vertex_shader = pipeline::load_shader_module(&api.device.handle, include_bytes!("../../../../../../resources/shaders/imgui/vertex.spv"), None);
    let fragment_shader = pipeline::load_shader_module(&api.device.handle, include_bytes!("../../../../../../resources/shaders/imgui/fragment.spv"), None);
...
fn create_font_atlas(context: &mut imgui::Context, api: &mut super::Renderer, descriptor_set: vk::DescriptorSet, sampler: vk::Sampler) -> (Image, vka::Allocation) {
    ...
    upload_buffer.upload(font_atlas.data, &mut upload_buffer_allocation, api.device.get_min_memory_map_alignment());
    ...
    unsafe { api.device.handle.cmd_copy_buffer_to_image(command_buffer, upload_buffer.handle, image.handle, vk::ImageLayout::TRANSFER_DST_OPTIMAL, &regions) };
    ...
    unsafe {
        ...
        //api.device.handle.destroy_buffer(upload_buffer.handle, None);
        //api.resource_allocator.handle.free(upload_buffer_allocation).expect("koi::ren::vk::imgui - failed to Free ImGui Font Upload Buffer");
    };

    (image, image_allocation)
}

fn setup_render_state(api: &mut super::Renderer, draw_data: &DrawData, index_buffer: vk::Buffer, vertex_buffer: vk::Buffer, pipeline_layout: vk::PipelineLayout, pipeline: vk::Pipeline, command_buffer: vk::CommandBuffer, frambuffer_width: f32, framebuffer_height: f32) {
    unsafe { api.device.handle.cmd_bind_pipeline(command_buffer, vk::PipelineBindPoint::GRAPHICS, pipeline) };

    if draw_data.total_vtx_count > 0 {
        unsafe {
            api.device.handle.cmd_bind_index_buffer(command_buffer, index_buffer, 0, if IDX_SIZE == 2 { vk::IndexType::UINT16 } else { vk::IndexType::UINT32 });
    ...
    unsafe {
        api.device.handle.cmd_push_constants(command_buffer, pipeline_layout, vk::ShaderStageFlags::VERTEX, F32_SIZE as u32 * 0, &scale_buffer);
        api.device.handle.cmd_push_constants(command_buffer, pipeline_layout, vk::ShaderStageFlags::VERTEX, F32_SIZE as u32 * 2, &translate_buffer);
    }
}