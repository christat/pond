...
impl Device {
    pub fn new(instance: &Instance, surface: &Surface) -> Self {
        let physical_devices = unsafe { instance.enumerate_physical_devices().expect("koi::ren::vk::Device - failed to enumerate physical devices") };
        ...
        let selected_physical_device = suitable_physical_devices.first().expect("koi::ren::vk::Device - failed to find suitable physical device");

        let mut device_config = config::DeviceConfig::new(&instance, selected_physical_device).expect("koi::ren::vk::Device - failed to create device config");
        ...
        let device = unsafe { instance.create_device(selected_physical_device.handle, &create_info, None).expect("koi::ren::vk::Device - failed to create device") };
