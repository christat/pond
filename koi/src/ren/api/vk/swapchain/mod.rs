...
impl Swapchain {
    pub fn new(instance: &Instance, device: &Device, surface: &Surface, resolution: &Resolution) -> Result<(Swapchain, SurfaceSupport), SwapchainError> {
    ...
    pub fn create(instance: &Instance, device: &Device, surface: &Surface, surface_support: &SurfaceSupport, resolution: &Resolution) -> Self {
        let surface_format = select_surface_format(&surface_support, vk::Format::B8G8R8A8_UNORM, vk::ColorSpaceKHR::SRGB_NONLINEAR);
        ...
        if !queue_family_indices.is_empty() { create_info = create_info.queue_family_indices(&queue_family_indices) };

        let swapchain_device = khr::swapchain::Device::new(&instance.handle, &device.handle);

        let khr = unsafe{ swapchain_device.create_swapchain(&create_info, None).expect("koi::ren::vk::swapchain - failed to create Swapchain") };
        let images = unsafe { swapchain_device.get_swapchain_images(khr).expect("koi::ren::vk::swapchain - failed to get swapchain Images") };
        let image_views: Vec<vk::ImageView> = images.iter().map(|swapchain_image| {
...
fn query_surface_support(physical_device: vk::PhysicalDevice, surface: &Surface) -> Result<SurfaceSupport, SwapchainError> {
    let capabilities = unsafe{ surface.instance.get_physical_device_surface_capabilities(physical_device, surface.khr).expect("koi::ren::vk::swapchain - failed to query physical device surface capabilities") };
    
    let formats = unsafe{ surface.instance.get_physical_device_surface_formats(physical_device, surface.khr).expect("koi::ren::vk::swapchain - failed to query physical device surface formats") }; 
    if formats.is_empty() { return Err(SwapchainError::NoSurfaceFormats); }
    
    let present_modes = unsafe{ surface.instance.get_physical_device_surface_present_modes(physical_device, surface.khr).expect("koi::ren::vk::swapchain - failed to query physical device surface present modes") };
    if present_modes.is_empty() { return Err(SwapchainError::NoPresentModes); }

    Ok(SurfaceSupport::new(capabilities, formats, present_modes))
}

fn select_surface_format(surface_support: &SurfaceSupport, desired_format: vk::Format, desired_color_space: vk::ColorSpaceKHR) -> vk::SurfaceFormatKHR {
    match surface_support.formats.iter().find(|&format| format.format == desired_format && format.color_space == desired_color_space) {
        Some(format) => format.clone(),
        None => surface_support.formats.first().unwrap().clone()
    }
}

fn select_present_mode(surface_support: &SurfaceSupport, desired_present_mode: vk::PresentModeKHR) -> vk::PresentModeKHR {
    match surface_support.present_modes.iter().find(|&&present_mode| present_mode == desired_present_mode) {
...
fn select_swapchain_extent(surface_support: &SurfaceSupport, resolution: &Resolution) -> vk::Extent2D {
    ...
    vk::Extent2D::default()
        .width(cmp::min(cmp::max(min_image_extent.width, resolution.width), max_image_extent.width))
        .height(cmp::min(cmp::max(min_image_extent.height, resolution.height), max_image_extent.height))
}
...
fn get_queue_family_config(device: &Device) -> (vk::SharingMode, Vec<u32>) {
    let Device{ queue_families: qf, .. } = device;
